...
{
	decaIrqStatus_t s = port_GetEXT_IRQStatus(pports);

	if(s) {
		port_DisableEXT_IRQ(pports); //disable the external interrupt line
	}
 ...
 * returns the state of the DW1000 interrupt
 */
void decamutexoff(decaIrqStatus_t s, UWBPortTypeDef *pports)        // put a function here that re-enables the interrupt at the end of the critical section
{
	if(s) { //need to check the port state as we can't use level sensitive interrupt on the STM ARM
		port_EnableEXT_IRQ(pports);
	}
}
